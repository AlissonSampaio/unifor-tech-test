...
class MatrizCurricularService {
    ...
    fun criar(request: MatrizCurricularRequest, keycloakId: String): MatrizCurricularResponse {
        ...
        val disciplina = disciplinaRepository.findById(discId)
            ?: throw NotFoundException("Disciplina", discId)

        val profId = request.professorId!!
        val professor = professorRepository.findById(profId)
            ?: throw NotFoundException("Professor", profId)
        ...
        if (cursosRemovidos.isNotEmpty()) {
            ...
            for (cursoRemovido in cursosRemovidos) {
                if (matriculas.any { it.aluno.curso.id == cursoRemovido.id }) {
                    throw ConflictException("Não é possível remover o curso ${cursoRemovido.nome} pois há alunos matriculados")
                }
            }
        }
    ...
    private fun toResponse(m: MatrizCurricular): MatrizCurricularResponse {
        ...
        return MatrizCurricularResponse(
            ...
            professor = ProfessorResponse(m.professor.id!!, m.professor.nome, m.professor.email),
            horario = HorarioResponse(m.horario.id!!, m.horario.diaSemana, m.horario.horaInicio, m.horario.horaFim, m.horario.periodo),
            maxAlunos = m.maxAlunos,
            vagasDisponiveis = (m.maxAlunos - matriculados.toInt()).coerceAtLeast(0),
            cursosAutorizados = m.cursosAutorizados.map { CursoResponse(it.id!!, it.codigo, it.nome) }
        )
    }
}
